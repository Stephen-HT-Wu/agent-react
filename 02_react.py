...
def run_react_loop(question: str, max_turns: int = 8) -> None:
    """手刻 ReAct 迴圈：Thought -> Action -> Observation，直到模型給出最終答案或達到 max_turns。
    ...
    2. for turn in range(max_turns):
        a. 呼叫 client.messages.create(
               model=MODEL, max_tokens=1024, system=SYSTEM_PROMPT,
               tools=TOOLS, messages=messages,
           )
        b. 把 response.content 裡的 text block 印出來，標成 "[Thought]"
           （這就是模型被系統提示要求、在呼叫工具前先講的推理）
        c. 如果 response.stop_reason != "tool_use"：
           印出最終答案的 text block，標成 "[Final Answer]"，然後 break
        d. 否則（stop_reason == "tool_use"）：
           - messages.append({"role": "assistant", "content": response.content})
           - 對 response.content 裡每一個 tool_use 類型的 block：
             * 印出 "[Action]"：block.name 和 block.input
             * result_str, is_error = call_tool(block.name, block.input)
             * 印出 "[Observation]"：result_str（如果 is_error 順便標註「⚠️ 錯誤」）
             * 組成 tool_result block：
               {"type": "tool_result", "tool_use_id": block.id,
                "content": result_str, "is_error": is_error}
           - 把這一輪蒐集到的所有 tool_result（可能不只一個）包成一個
             user message，一次 append 進 messages，才進入下一輪迴圈

    3. 如果迴圈跑完 max_turns 還沒進到步驟 2c 的 break，
       印出「⚠️ 超過 max_turns，agent 可能卡住了」——
       這是刻意設計的失敗模式，讓你看見 ReAct 沒有終止條件時會怎麼鬼打牆。

    寫完後跑 main()，對照驗收標準逐條檢查印出來的軌跡。
    """
    raise NotImplementedError("把手刻的 ReAct 迴圈寫在這裡")
