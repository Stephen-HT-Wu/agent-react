...
測試題（PLAN.md 原題）：
  「現在晚上 11 點在逢甲，資料集裡的宵夜哪家還開著而且最近？」

這題裡藏了一個跟階段 1 類似的陷阱，但這次是「事實陷阱」而不是「推理陷阱」：
  河南夜宵生魚以「半夜 12 點才開門」聞名（資料集裡的宵夜代表），
  但晚上 11 點還沒到午夜，它其實還沒開——如果 agent 只做關鍵字比對、
  沒有真的呼叫 is_open() 驗證，很容易直接答錯成河南夜宵生魚。
  正確答案要從真正開著的幾家店（逢甲香腸舖、巷弄臭豆腐、紅燒當歸鴨麵、靈魂乾麵店）
...
驗收標準：
  ...
  [ ] 故意讓一個工具回傳錯誤（把某個工具呼叫裡的店名打錯），
      ...
      直接呼叫 call_tool("is_open", {"shop_name": "巷弄臭都腐", "time": "23:00"})
...
def _find_shop(name: str) -> dict:
    for s in ALL_SHOPS:
        if s["name"] == name:
            return s
    candidates = difflib.get_close_matches(name, [s["name"] for s in ALL_SHOPS], n=1, cutoff=0.4) # 找出最相似的店名
    hint = f"，你是不是要找「{candidates[0]}」？" if candidates else "" # 如果找不到，提示最相似的店名
    raise LookupError(f"找不到店名「{name}」{hint}") # 如果找不到，抛出异常
...
def _haversine_km(lat1: float, lng1: float, lat2: float, lng2: float) -> float:
    r = 6371.0
    phi1, phi2 = math.radians(lat1), math.radians(lat2)# 將經緯度轉換為弧度
    dphi = math.radians(lat2 - lat1)# 計算經度差
    dlambda = math.radians(lng2 - lng1)# 計算緯度差
    a = math.sin(dphi / 2) ** 2 + math.cos(phi1) * math.cos(phi2) * math.sin(dlambda / 2) ** 2# 計算haversine公式中的a
    return 2 * r * math.asin(math.sqrt(a))# 計算haversine公式中的距離
...
def is_open(shop_name: str, time: str) -> dict:
    ...
    if open_m <= close_m:# 如果營業時間在午夜12點之前
        is_open_now = open_m <= t_m <= close_m
    else:  # 如果營業時間在午夜12點之後
        is_open_now = t_m >= open_m or t_m <= close_m# 如果營業時間在午夜12點之後，則判斷是否營業中
...
def distance(point_a: str, point_b: str) -> dict:
    ...
    km = _haversine_km(lat1, lng1, lat2, lng2)# 計算兩個地點之間的直線距離
...
TOOLS = [
    {
        "name": "search_shops",
        "description": "依城市（和可選的餐點/店名關鍵字）查詢資料集裡的店家清單，回傳店名、餐點、營業時間、價位、地區、備註。",
        "input_schema": {
            "type": "object",
            "properties": {
...
TEST_QUESTION = "現在晚上 11 點在逢甲，資料集裡的宵夜哪家還開著而且最近？"
