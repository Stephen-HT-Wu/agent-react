...
用 data/docs/*.md（20 篇店家介紹文）做三種版本的對照：
  A 版：完全不給文件（幻覺測試基準線）
...
已經換成資料裡實際存在的細節）：
  「資料集裡的碗粿老店中，哪一家只賣碗粿和魚羹兩種東西、均一價35元？
   出自哪一篇店家介紹？」
  正解：永樂碗粿店，出自〈永樂碗粿店〉介紹。
...
額外挑戰題（已經實測驗證會撈錯，留給你自己手動重現）：
  ...
  正解是溪邊雞肉飯（05:30-13:00，文件裡寫「清晨五點半就開賣」「晚來就賣完」），
  但這題刻意用同義不同字面的說法問（「只做早上生意」「去晚一點就撲空」），
  跟文件實際用字（清晨、晚來、賣完）完全沒有重疊——結果 retrieve_top_k() 撈到的
  top-3 是民主路、郭氏、霞仔，溪邊雞肉飯直接沒進榜。這就是 TF-IDF 的天生弱點：
...
TEST_QUESTION = "資料集裡的碗粿老店中，哪一家只賣碗粿和魚羹兩種東西、均一價35元？出自哪一篇店家介紹？"
...
def main() -> None:
    ...
    print("[ ] C 版有沒有附上正確出處（永樂碗粿店 + 〈永樂碗粿店〉介紹）？")
    print()
    print(f"[ ] 額外挑戰：retrieve_top_k('{FOLLOW_UP_QUESTION}') 撈到的 top-3")
    print("    有沒有包含溪邊雞肉飯（正解）？已驗證這題會撈錯（溪邊掉出榜外），")
