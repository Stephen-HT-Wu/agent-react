...
TAICHUNG_SHOPS = [s for s in ALL_SHOPS if s["city"] == "台中"]
...
def plan_route(shop_names: list[str], start: str = "台中車站") -> dict:
    ...
    remaining = [s for s in shop_names if s in TAICHUNG_SHOP_NAMES]
    if not remaining:
    ...
    while remaining:
        ...
        seg = distance(current, nearest)
        transit = estimate_transit(current, nearest)
        segments.append(transit)
...
def run_subagent(
    ...
    """執行一個子 agent 的 ReAct 迴圈。messages 只在這個函式裡存在——context 隔離。"""
    print(f"\n{'=' * 60}")
    ...
    for turn in range(max_turns):
        ...
        for block in response.content:
            ...
            tag = "⚠️ 錯誤：" if is_error else ""
            print(f"[{agent_name} Observation] {tag}{result_str}")
            tool_results.append({
                "type": "tool_result",
