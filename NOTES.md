...
2. 府前冰果室週三公休——TASK 指定「這個週三出發」，若還排進去，代表沒有
...
欄位裡剛好寫著「逢甲」兩個字的 3 家店（逢甲香腸舖、紅燒當歸鴨麵、河南夜宵生魚），
完全沒發現巷弄臭豆腐和靈魂乾麵店 也在同一個商圈——因為它們的備註寫的是
「文華路5巷」「福上巷」，沒有字面上的「逢甲」。
...
|---|---|---|
| **巷弄臭豆腐** | ✅ | **0.13 km（真正最近）** |
| 逢甲香腸舖 | ✅ | 0.19 km（agent 最終選的答案） |
| 紅燒當歸鴨麵 | ✅ | 0.26 km |
| 靈魂乾麵店 | ✅ | 0.32 km |
| 河南夜宵生魚 | ❌（半夜12點才開，11點還沒到） | 0.65 km |

Agent 正確避開了「河南夜宵生魚」這個知名度陷阱（有確實呼叫 `is_open()` 驗證，
沒有單憑「傳奇宵夜」的印象就選它），但選出的「逢甲香腸舖」不是真正最近的——
...
這比原本設計的「河南夜宵生魚陷阱」更完整地回答了驗收標準最後一題——ReAct 跟階段 1
...
print(m.call_tool('is_open', {'shop_name': '巷弄臭都腐', 'time': '23:00'}))
"
```
...
候選再篩選，看看修正搜尋策略後答案會不會變成正確的巷弄臭豆腐。這也是階段 4
...
測試題：「資料集裡的碗粿老店裡，哪一家只賣碗粿和魚羹兩種東西、均一價35元？出自哪一篇店家介紹？」

正解：永樂碗粿店，出自〈永樂碗粿店〉。「均一價35元」和「哪篇報導」都是
...
```
TF("碗粿", 永樂碗粿店文件) = 「碗粿」出現次數 / 這篇總 bigram 數
```
...
正解是溪邊雞肉飯（05:30-13:00）。[`data/docs/溪邊雞肉飯.md`](data/docs/溪邊雞肉飯.md)
...
同時，問題裡的「嘉義」「雞肉飯」會跟**很多**嘉義雞肉飯文件重疊——民主路、郭氏、霞仔
都會被拉高，溪邊反而因為「早鳥、清晨五點半」這種獨特用字，跟問題的用字對不上。

結果 `retrieve_top_k()` 撈到的 top-3 是民主路、郭氏、霞仔，溪邊雞肉飯直接沒進榜。
...
① 委派美食評論員（不知預算）
② 委派預算控管（只收到店名清單）
③ 若餐飲預估 > 1000 → 衝突仲裁，請評論員替換最貴的店（如河南夜宵生魚）
④ 委派交通規劃師（只收到最終店名）
⑤ 行程總監彙整兩天一夜行程
```

刻意設計的衝突：評論員若把**河南夜宵生魚**排第一，5 家店輪流吃 6 餐會讓河南夜宵生魚吃兩次
...
| 2 | `recommend_food_itinerary(...)` | 本地 RAG | 老城鹹粥、保安豬心肺粉、府前冰果室 |
...
- **文件內容**：C 版若回傳大段 `data/docs` 原文，問幾百次可能拼回 20 篇。
  （本專案資料為練習用途之虛構整理，機密價值有限，但「整理方式與 schema」仍有價值。）
